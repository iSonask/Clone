...
import QuartzCore
import ObjectiveC
//https://github.com/mo-badi3/iOS-Multi-Picker
/*
 ...
 MultiPickerDialog().show(title: "Select School",doneButtonTitle:"Done", cancelButtonTitle:"Cancel" ,options: pickerData, selected:  selectedIDs) {
 values -> Void in
 ...
 //                    selectedIDs.append(value["value"]!)
 finalText = finalText  + value["display"]! + (index < values.count - 1 ? " , ": "")
 }
 self.schoolTextField.text = finalText
 }
 */
class MultiPickerDialog: UIView, UITableViewDelegate, UITableViewDataSource {
    ...
    func show(title: String, doneButtonTitle: String = "Select", cancelButtonTitle: String = "Cancel", options: [[String: String]], selected: [String]? = nil, callback: @escaping MultiPickerCallback) {
        ...
        if selected != nil {
            ...
            for index in selectedIndices{
                let selectedCellIndexPath = IndexPath(row: index, section: 0)//NSIndexPath.init(forRow: index, inSection: 0)
                self.tableView(picker, didSelectRowAt: selectedCellIndexPath)
                picker.selectRow(at: selectedCellIndexPath, animated: true, scrollPosition: .none)
            }
        }
    ...
    private func createContainerView() -> UIView {
        ...
        self.frame = CGRect(x: 0, y: 0, width: screenSize.width, height: screenSize.height) // CGRectMake(0, 0, screenSize.width, screenSize.height)
        
        // This is the dialog's container; we attach the custom content and the buttons to this one
        let dialogContainer = UIView(frame: CGRect(x: (screenSize.width - dialogSize.width) / 2, y: (screenSize.height - dialogSize.height) / 2, width: dialogSize.width, height: dialogSize.height)) //UIView(frame: CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - dialogSize.height) / 2, dialogSize.width, dialogSize.height))
        ...
        dialogContainer.layer.shadowOffset = CGSize(width: 0 - (cornerRadius + 5) / 2, height: 0 - (cornerRadius + 5) / 2)//CGSizeMake(0 - (cornerRadius + 5) / 2, 0 - (cornerRadius + 5) / 2)
    ...
    private func addButtonsToView(container: UIView) {
        ...
        self.cancelButton.addTarget(self, action: #selector(MultiPickerDialog.buttonTapped(_:)), for: UIControl.Event.touchUpInside)
        ...
        self.doneButton.addTarget(self, action: #selector(buttonTapped(_:)), for: UIControl.Event.touchUpInside)
        ...
        if sender.tag == kPickerDialogDoneButtonTag {
            ...
            if let indexPathsForSelectedRows = self.picker.indexPathsForSelectedRows{
                for indexPath  in indexPathsForSelectedRows{
                    ...
                    theSelectedValues.append(["display":(cell?.textLabel?.text)!])
                }
            }
    ...
    func countScreenSize() -> CGSize {
        ...
        return CGSize(width: screenWidth, height: screenHeight)//CGSizeMake(screenWidth, screenHeight)
    }
